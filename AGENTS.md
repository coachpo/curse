...
├── spear/               # Spear app (Postgres + Django backend + worker + Next.js frontend)
...
- **Spear `change-me` placeholders** — `POSTGRES_PASSWORD`, `DJANGO_SECRET_KEY`, `JWT_SIGNING_KEY` in `spear/env.example` must be replaced.
